...
2025.11.13
新增经纬度提取、页面范围自动获取、断点续爬、一次性所有区域价位爬取、自定义位置爬取（会接着剩下的区域、价位、页数继续爬）

具体只需要改一下main的代码即可，已更新上传，爬取过程设计的稍慢防止被反爬措施限制（可以加入cookie加强反爬），如要快速建议加入代理池。

代码加入了开局验证机制，有效避免开局错误。

                     #########################################################################################
                     ##       总而言之，现在只需要设置你要爬取城市的链接、数据库，后续不需要再进行设置了     ###
                     #########################################################################################
