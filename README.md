# 2025.12.4(新修改）<br>
# 修复爬取一定数量后出现乱码的问题，只需要下载获取main（乱码修复版）的代码即可，如果中途出错试试main的代码，修复代码只是简单的测试了一下。

# 2025.11.15(修改）<br>
---------
# 1.不需要Cookie也能运行，但是容易被反爬，最好是先网页登录，再在代码中session.headers.update加入Cookie的设置，如果运行过程后续验证正常但是没有数据可能是转到登录页面了，建议试试打开网页刷新一下验证，然后重新设置自定义位置接着爬。

## 新增经纬度提取、页面范围自动获取、断点续爬、一次性所有区域价位爬取、自定义位置爬取（会接着剩下的区域、价位、页数继续爬）具体只需要改一下main的代码即可，已更新上传，爬取过程设计的稍慢防止被反爬措施限制（可以加入cookie加强反爬），如要快速建议加入代理池。<br>

## 加入了价格提取，代码加入了开局验证机制，有效避免开局错误，注意遇到的大多数错误基本都是要去手动点击验证<br>
